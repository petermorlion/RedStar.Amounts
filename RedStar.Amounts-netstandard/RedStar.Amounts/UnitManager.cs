...
namespace RedStar.Amounts
{
    ...
    /// to resolve units.
    /// </summary>
    public delegate Unit UnitResolveEventHandler(object sender, UnitResolveEventArgs args);
    ...
    public sealed class UnitManager
    {
        ...
        public static void RegisterConversions(Type unitConversionsClass)
        {
            ...
            foreach (MethodInfo method in unitConversionsClass.GetRuntimeMethods().Where(x => x.IsPublic && x.IsStatic))
            {
        ...
        public static void RegisterConversions(Assembly assembly)
        {
            ...
            {
                if (t.GetTypeInfo().GetCustomAttributes(typeof(UnitConversionClassAttribute), false).Any())
                {
        ...
        {
            foreach (var field in unitDefinitionClass.GetRuntimeFields().Where(x => x.IsPublic && x.IsStatic))
            {
        ...
        public static void RegisterUnits(Assembly assembly)
        {
            ...
            {
                if (t.GetTypeInfo().GetCustomAttributes(typeof(UnitDefinitionClassAttribute), false).Any())
                {
                    RegisterUnits(t);
                }
            }
...
#region Public methods - Named units
        ...
        public static bool TryGetUnitByName(string name, out Unit unit)
        {
            ...
            {
                unit = handler(Instance, new UnitResolveEventArgs(name));
                if (unit == null)
                {
                    continue;
                }
